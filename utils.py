...
def load_mnist_trainval():
    ...
    train_label = label[:tr_size]
    val_data = data[tr_size:]
    val_label =label[tr_size:]
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    #############################################################################
    if shuffle:
      both = list(zip(data,label))
      random.shuffle(both)
      data, label = zip(*both)
    batched_data = []
    batched_label = []
    for i in range(0,len(data),batch_size):
      batched_data.append(np.asarray(data[i:batch_size+i]))
      batched_label.append(np.asarray(label[i:batch_size+i]))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.plot(epochs, train_loss_history, c = "b")
    plt.plot(epochs, valid_loss_history,c = "r")
    plt.legend(["Train","Valid"])
    plt.title("Loss Curve")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.show()
    plt.plot(epochs, train_acc_history, c = "b")
    plt.plot(epochs, valid_acc_history,c = "r")
    plt.legend(["Train","Valid"])
